...
- [ ] **A1 Kelime Verisi: Yeni Kategori (colors_and_numbers)** — `src/pages/Vocabulary/A1/data.tsx` dosyasındaki `vocabulary` objesine mevcut formatla aynı `colors_and_numbers` kategorisi ekle (6 kelime: red, blue, green, one, two, three gibi EN TEMEL kelimeler). Kelimeleri ve Türkçe anlamlarını `scripts/cefr/turkce_kelimeler.json` dosyasından A1 seviyesinden seç, `tr` alanını BİREBİR kullan (kendin uydurma). `example_sentences` en az 2, `questions` en az 2. Mevcut yapıyı ve `key` sıralamasını birebir koru. Bu kategori EN YENİ BAŞLAYAN kullanıcı için — kelimeler çok basit olsun.
- [ ] **A2 Kelime Verisi: Yeni Kategori (food_and_drink)** — `src/pages/Vocabulary/A2/data.tsx` dosyasındaki mevcut yapıya `food_and_drink` kategorisi ekle (6 kelime: bread, water, milk, apple, rice, chicken). Kelime + Türkçe anlam + örnek cümleyi `scripts/cefr/turkce_kelimeler.json` dosyasından A2 seviyesinden BİREBİR al. `example_sentences` en az 2, `questions` en az 2. Mevcut JSON/TS yapısını birebir koru, tasarımı değiştirme.
- [ ] **B1 Kelime Verisi: Yeni Kategori (shopping)** — `src/pages/Vocabulary/B1/data.tsx` dosyasındaki `vocabulary` objesine `shopping` kategorisi ekle (6 kelime: price, buy, sell, cheap, expensive, shop). Kelime + Türkçe anlam + örnek cümleyi `scripts/cefr/turkce_kelimeler.json` dosyasından B1 seviyesinden BİREBİR al (`tr` ve `example` alanları). `key` numaralarını sırayla devam ettir. Mevcut yapıyı birebir koru.
...
- [x] **Quiz Kullanıcı Adı Girişi** — `src/pages/games/quiz/GameOverModal.tsx` dosyasında skor kaydı öncesi kullanıcıdan isim alan bir `TextInput` ekle. `leaderData.ts` yapısına uygun kayıt oluştur. Modal yapısını bozma.
- [x] **WordCompletion Geri Bildirimi** — `src/pages/games/wordcompletion/WordCompletion.tsx` dosyasında cevap doğru/yanlış olduğunda ekranda renkli bir geri bildirim metni göster. Mevcut `styles.tsx` yapısına uygun stil ekle, oyun mantığını değiştirme.
- [x] **FlappyBird Yüksek Skor Kaydı** — `src/pages/games/flappybird/` dosyalarını incele, AsyncStorage ile en yüksek skoru kaydet ve oyun sonunda göster. Paket kurulumu GEREKMİYOR — uygulamada AsyncStorage zaten kurulu mu kontrol et, yoksa `@react-native-async-storage/async-storage` importu kullanılıyorsa doğrudan kullan.
...
- [ ] **Basit Kullanıcı Modu (Yeni Başlayan Rehberi)** — Uygulamanın ana ekranına (BottomTab ana sayfa) ilk açılışta 3 adımlı basit bir rehber (onboarding tooltip) ekle: 1) "Buradan kelime öğrenirsin", 2) "Buradan oyun oynarsın", 3) "Her kelimenin Türkçe anlamı ve örnek cümlesi var". `AsyncStorage` ile `welcome_seen` flag'ine bak, görüldüyse gösterme. Renk ve stil: mevcut tema (koyu arka plan, vurgu rengi). Modal yerine altta sade bilgi kartı kullan — kullanıcıyı boğma. Mevcut ana ekran yapısını bozma.
- [ ] **Kelime Çeviri Kalite İyileştirme** — `src/pages/Vocabulary/` ekranlarında kelime kartına dokununca kelimenin TÜRKÇE anlamının ve örnek cümlesinin büyük ve okunaklı gösterildiğini doğrula. `src/components/quizComponent/` içindeki quiz modülünde soru kelimesinin altına Türkçe anlamını küçük ipucu olarak ekle (ipucu butonu: soru ekranında "İpucu" butonu, basınca Türkçe anlamı 3 saniye göster). Mevcut tasarımı koru, sadece okunabilirlik/erişilebilirlik ekle. Paket kurulumu GEREKMEZ.
- [ ] **Sesli Telaffuz Butonu (TTS)** — Kelime kartlarında ve quiz sorularında kelimenin yanına 🔊 sesli okuma butonu ekle. `react-native-tts` paketi kuruluysa kullan; kurulu değilse `expo-speech` dene; hiçbiri yoksa `expo-speech` kur (`npx expo install expo-speech`). Buton kelimeyi İngilizce telaffuzla okusun. Mevcut kart tasarımını bozmadan butonu sağ üste ekle. TTS sesi kadar kaliteli olmalı — bu PREMIUM uygulamanın olmazsa olmazı.
