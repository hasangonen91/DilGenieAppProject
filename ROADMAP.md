...
- [ ] **LivePatch Entegrasyonu (Anında Güncelleme)** — `react-native-livepatch` paketini (Hasan'ın paketi, CodePush'un ücretsiz halefi) uygulamaya entegre et: (1) `yarn add react-native-livepatch` ile ekle; (2) App açılışında `LivePatch.configure({ updateUrl: 'https://github.com/hasangonen91/DilGenieAppProject/releases/latest/download', channel: 'production' })` çağır; (3) `LivePatch.sync()` ile otomatik güncelleme kontrolü yap; (4) Android `MainApplication.kt`'e native entegrasyonu ekle (LivePatchModule.getBundleUrl); (5) iOS `AppDelegate.swift`'e bundleURL entegrasyonu ekle. JS/asset güncellemeleri mağaza onayı olmadan anında telefona insin.
